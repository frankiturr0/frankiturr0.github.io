...
df_filtrado <- senado_nomina %>%
  filter(voto_al_dnu %in% c("A favor", "Posible a favor", "En negociación", "Indefinido", "Posible en contra"))
...
grafico_resumen_votos_plotly <- plot_ly(resumen_votos, x = ~factor(voto_al_dnu, levels=c("A favor", "Posible a favor", "En negociación", "Indefinido", "Posible en contra", "En contra")), y = ~cantidad, type = 'bar', marker = list(color = resumen_votos$colour)) %>%
...
resumen_votos <- senado_nomina %>%
  ...
  summarise(cantidad = n(), 
            color = unique(colour),
            votos_favor = sum(voto_al_dnu == "A favor"),
            votos_contra = sum(voto_al_dnu == "En contra"),
            votos_posible_favor = sum(voto_al_dnu == "Posible a favor"),
            votos_posible_contra = sum(voto_al_dnu == "Posible en contra"))

# Crear el gráfico interactivo de barras
grafico_resumen_votos_plotly <- plot_ly(resumen_votos, 
                                        x = ~factor(party_long, levels=c("Unidad Ciudadana","Frente Nacional y Popular","Frente Renovador de la Concordia Social","Juntos Somos Río Negro","Unidad Federal","Por Santa Cruz","Unión Cívica Radical","Partido por la Justicia Social","Frente PRO","Cambio Federal","La Libertad Avanza","Comunidad Neuquén")), 
                                        y = ~cantidad, 
                                        type = 'bar', 
                                        hoverinfo = "text",
                                        text = ~paste("Votos a favor:", votos_favor, "<br>",
                                                      "Votos en contra:", votos_contra, "<br>",
                                                      "Posible a favor:", votos_posible_favor, "<br>",
                                                      "Posible en contra:", votos_posible_contra, "<br>"),
                                        marker = list(color = resumen_votos$color)) %>%
  layout(title = "Resumen de Votos del Senado sobre el DNU 70/23",
         xaxis = list(title = "Bloque"),
         yaxis = list(title = "Cantidad de votos"),
         showlegend = FALSE)
...
### Tabla con información pública de los senadores que están a favor o indecisos de la Ley Ómnibus

``` {r}

# Display the entire dataframe as an HTML table with styling
knitr::kable(
  df_filtrado %>% select(-voto_ley_omnibus, -seats, -colour, -orden),
  format = "html",
